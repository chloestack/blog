...
매일 새벽 02:00에 로컬 launchd 잡(`com.tistory-writer.daily`)이
...
| --- | --- |
| 접속 국가가 KR | 한국어 그대로 |
| 접속 국가를 모름(헤더 없음, `XX`, `T1`) | 한국어 그대로. Accept-Language는 보지 않습니다 |
| Accept-Language에 한국어가 있음 | 한국어 그대로 (해외 거주 한국어 사용자) |
| 크롤러(Googlebot 등) | 한국어 그대로. 짝은 hreflang으로만 알립니다 |
| `lang` 쿠키로 한국어를 고른 사람 | 한국어 그대로 |
| 사이트 안에서 링크를 눌러 온 사람 | 한국어 그대로. 지역 추측은 첫 진입에서만 합니다 |
| 그 밖의 해외 방문자 | `/en/posts/<영문 slug>`로 302 |
| 번역본이 없는 글 | 옮길 곳이 없으므로 한국어 그대로 |

지역 추측은 **밖에서 처음 들어올 때만** 합니다 — Referer의 host가 우리 사이트와
같으면 그대로 둡니다. 읽는 도중에 지면이 영문으로 갈아엎히지 않게 하려는 것이고,
번역본이 있는 글과 없는 글이 섞여 있어 글마다 튀고 안 튀고가 갈리던 문제도 이걸로
사라집니다. `lang` 쿠키로 직접 고른 언어는 이와 무관하게 사이트 안에서도 계속
듣습니다.
