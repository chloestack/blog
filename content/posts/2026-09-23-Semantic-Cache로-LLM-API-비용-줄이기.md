...
임베딩(Embedding)은 텍스트를 고차원 실수 벡터로 변환하는 표현 방식입니다. OpenAI `text-embedding-3-small` 기준으로 각 문장은 1536차원 벡터로 표현되며, 의미적으로 유사한 문장들은 이 벡터 공간에서 서로 가까이 위치합니다. "비밀번호를 잊어버렸어요"와 "계정 비밀번호를 모르겠습니다"는 전혀 다른 토큰 시퀀스이지만, 임베딩 공간에서는 코사인 유사도 0.93~0.96 수준으로 매우 가깝습니다. 반면 "비밀번호 변경"과 "배송 조회"는 0.2~0.3 수준에 그칩니다.
