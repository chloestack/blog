...
자주 같이 묶여 언급되지만, **Vercel에는 JVM 런타임이 없습니다.** 함수 런타임은 Node.js·Python·Go·Ruby이고 Java는 없습니다. 그래서 Vercel은 Spring Boot의 대안이 아니라 **앞단 조합**입니다.
...
- **Supabase Free** — 1주일 비활동 시 프로젝트가 **일시 정지**됩니다. 사이드 프로젝트를 가끔 켜보는 용도라면 매번 깨우게 됩니다.
- **Render Free** — 15분 무요청이면 인스턴스가 내려갑니다. 그리고 Spring Boot는 **다시 뜨는 데 20~40초**가 걸립니다. 첫 방문자가 매번 그 시간을 기다립니다. 이 조합은 특히 나쁩니다.
...
- 서울 리전 기준 Spring Boot 서비스 하나는 **취미 7,700원 / 1인 실서비스 63,500원 / 이중화 프로덕션 623,700원**입니다.
...
- 소규모에서는 **Railway 43,600원 / Fly.io 30,700원 / Render·Supabase 59,000원**으로 구성 B보다 싸고, 배포·TLS·모니터링이 딸려 옵니다.
- 다만 **Railway의 vCPU 단가는 EC2의 2.6배**입니다. CPU를 상시 쓰는 앱이 늘면 역전됩니다.
...
10. [Vercel 요금](https://vercel.com/pricing) 및 [지원 런타임 문서](https://vercel.com/docs/functions/runtimes) — Java 런타임이 없다는 근거.
11. [Railway 요금](https://railway.com/pricing) — vCPU·RAM·볼륨·이그레스 단가.
12. [Render 요금](https://render.com/pricing) — Web Service와 Postgres 플랜.
13. [Fly.io 요금](https://fly.io/docs/about/pricing/) — CPU·RAM·볼륨 단가.
14. [Supabase 요금](https://supabase.com/pricing) — Pro 플랜과 Free 티어 일시 정지 정책.
